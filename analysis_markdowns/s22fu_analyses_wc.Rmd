...
source(paste0(path_to_bv,"code/functions/arlbv_utilities.R"))
...
# Add pairs col
new_trials_list <- by(trials,trials$sub_index,create_pair_cols_sub_s2)
...
```{r}
mpan_scds_intervals <- create_interval_plot(arr = mpan_scds,names = c("scd_beta_mu","scd_aff_sens_mu"),
                                          xmin = -.012,xmax = 2) +
                        theme(panel.grid.major.x = element_line(color = "#DCDCDC", size = 1.87),
                              panel.background = element_rect(fill = "white", color = NA),
                              axis.text.x = element_blank(),
                              axis.text.y=element_blank())
mpan_scds_intervals
ggsave("/Users/dp/Documents/conferences/SASolina_2025/figures/mpan_scds_intervals.pdf",mpan_scds_intervals,width=2,height=1)
...
```{r}
# Get necessary data
trials_bqba_sim <- trials %>% select(sub_index,block,trial_nl,fA_ix,fB_ix,feed_probe_number,out_a,out_b)
...
save(sim_effs,file=paste0(path_to_project_directory,"output/s2_aff_only_sim_effs.Rdata"))
```

Next, we run these same approximate analyses on the real data
```{r}
# Get necessary data
trials_bqba_real <- trials %>% select(sub_index,block,trial_nl,fA_ix,fB_ix,feed_probe_number,out_a,out_b,
                                      choice_numeric,prev_rate,feed_rate_z,chosen_frac,unchosen_frac,
                                      chosen_out,unchosen_out) %>%
                               rename(prat = prev_rate, choice = choice_numeric)
                               

# model_pred_arl_nonuis <- read_fsml("model_pred_arl_nonuis",model_out_dir=model_out_dir)
# Add parameter means needed for approximate data analysis
trials_bqba_real <- add_param_means(model_pred_arl_nonuis$sum,"alpha",trials_bqba_real)
trials_bqba_real <- add_param_means(model_pred_arl_nonuis$sum,"tau",trials_bqba_real)
trials_bqba_real <- add_param_means(model_pred_arl_nonuis$sum,"forget",trials_bqba_real)

real_effs <- est_bq_ba_s2(trials_bqba_real)
save(real_effs,file=paste0(path_to_project_directory,"output/s2_aff_only_real_effs.Rdata"))
```

Finally, we plot the simulated and actual effects of Q and A on choice
```{r}
# Plot effects of Q on choice
bq_sims <- ggplot(sim_effs, aes(x = bQs)) +
  ...
  geom_vline(xintercept = real_effs$bQ, color = "red", linewidth = 1) +
            theme_minimal() +
            theme(
...
ggsave("/Users/dp/Documents/manuscripts/vrv/figures_tables/supp_fig_2a_s2.pdf",bq_sims,width=2,height=1.33)
```

```{r}
# Plot effects of A on choice
ba_sims <- ggplot(sim_effs, aes(x = bAs)) +
            geom_histogram(fill = "steelblue", color = "black") +  # Adjust binwidth as needed
            geom_vline(xintercept = 0, color = "black", size = 1) +  # Black vertical line at 0
            geom_vline(xintercept = real_effs$bA, color = "red", size = 1) +
                      theme_minimal() +
                      theme(
                        panel.grid.major.y = element_blank(),  # Remove major vertical grid lines
                        panel.grid.minor.y = element_blank(),      
...
ggsave("/Users/dp/Documents/manuscripts/vrv/figures_tables/supp_fig_2b_s2.pdf",ba_sims,width=2,height=1.33)
```

# Valence models with alternative formulations of pre-trial reward expectations
These models are the same as the best-fitting model of valence, except they replace V_block with different
estimates of pre-trial reward expectations.

V_block replaced with the average reward in the block
```{r}
combined_shrink_av_vblock <- fit_stan_model(stan_file = paste0(stan_model_dir_s22fu,"combined_shrink_av_vblock.stan"),
                         model_out_dir = model_out_dir,
                         raw_data = trials,
                         study = "s22fu",
                         skip=c("waic","check_csvs"),
...
V_block replaced with the average reward in the task 
```{r}
combined_shrink_av_vtask <- fit_stan_model(stan_file = paste0(stan_model_dir_s22fu,"combined_shrink_av_vtask.stan"),
                         model_out_dir = model_out_dir,
                         raw_data = trials,
                         study = "s22fu",
                         skip=c("waic","check_csvs"),
...
V_block replaced with a weighted average of Q-values
```{r}
combined_shrink_onpolV <- fit_stan_model(stan_file = paste0(stan_model_dir_s22fu,"combined_shrink_onpolV.stan"),
                         model_out_dir = model_out_dir,
                         raw_data = trials,
                         study = "s22fu",
                         skip=c("waic","check_csvs"),
...
V_block calculated using an independent learning rate
```{r}
combined_shrink_freeVlr <- fit_stan_model(stan_file = paste0(stan_model_dir_s22fu,"combined_shrink_freeVlr.stan"),
                         model_out_dir = model_out_dir,
                         raw_data = trials,
                         study = "s22fu",
                         skip=c("waic","check_csvs"),
...
Get plots with theory vector lengths based on all of these models
```{r}
freelr_vec_fits <- get_vec_lens("combined_shrink_freeVlr",vars=c("dpiw_mu","fpiw_mu"),model_out_dir=model_out_dir)
avblock_vec_fits <- get_vec_lens("combined_shrink_av_vblock",vars=c("dpiw_mu","fpiw_mu"),model_out_dir=model_out_dir)
avtask_vec_fits <- get_vec_lens("combined_shrink_av_vtask",vars=c("dpiw_mu","fpiw_mu"),model_out_dir=model_out_dir)
onpol_vec_fits <- get_vec_lens("combined_shrink_onpolV",vars=c("dpiw_mu","fpiw_mu"),model_out_dir=model_out_dir)

ggsave(paste0(fig_dir,"onpol_lens_s2.pdf"),onpol_vec_fits$theor_mn_plot,width=1.8,height=1.35)
ggsave(paste0(fig_dir,"vblock_lens_s2.pdf"),avblock_vec_fits$theor_mn_plot,width=1.8,height=1.35)
ggsave(paste0(fig_dir,"vtask_lens_s2.pdf"),avtask_vec_fits$theor_mn_plot,width=1.8,height=1.35)
ggsave(paste0(fig_dir,"freelr_lens_s2.pdf"),freelr_vec_fits$theor_mn_plot,width=1.8,height=1.35)
```

# Fitting model with just q-learning

```{r}
q_only <- fit_stan_model(stan_file = paste0(stan_model_dir_s22fu,"q_only.stan"),
                         model_out_dir = model_out_dir,
                         raw_data = trials,
                         study = "s22fu",
                         skip=c("waic","check_csvs"),
...
```{r}
scd_beta_mu_q_only <- get_draws("q_only",model_out_dir=model_out_dir,vars=c("scd_beta_mu"))
get_ci_pd(draws = scd_beta_mu_q_only)
```

```{r}
alpha_mu_q_only <- get_draws("q_only",model_out_dir=model_out_dir,vars=c("alpha_mu"))
get_ci_pd(draws = sigmoid(alpha_mu_q_only))
```
...
```{r}
s22fu_qa_term_out <- fit_stan_model(stan_file = paste0(stan_model_dir_s22fu,"s22fu_qa_term_out.stan"),
                         model_out_dir = model_out_dir,
                         raw_data = trials,
                         study = "s22fu",
                         n_t=104)
...
# No-autocorrelation model variants

Fit no-choice-autocorrelation versions of the main affect and choice models.
```{r}
combined_shrink_nocac <- fit_stan_model(stan_file = paste0(stan_model_dir_s22fu,"combined_shrink_nocac.stan"),
                         model_out_dir = model_out_dir,
                         raw_data = trials,
                         study = "s22fu",
                         n_t=104)

mpan_nocac <- fit_stan_model(stan_file = paste0(stan_model_dir_s22fu,"mpan_nocac.stan"),
                         model_out_dir = model_out_dir,
                         raw_data = trials,
                         study = "s22fu",
                         n_t=104)
...
Assess effects of Q and A on choice from the no-autocorrelation choice model posterior.
```{r}
mpan_nocac_scds <- get_draws("mpan_nocac",model_out_dir=model_out_dir,vars=c("scd_beta_mu","scd_aff_sens_mu"))
get_ci_pd(draws = mpan_nocac_scds[,,"scd_beta_mu"])
get_ci_pd(draws = mpan_nocac_scds[,,"scd_aff_sens_mu"])
```

Estimate no-autocorrelation vector lengths for reward, prediction error, and counterfactual comparison.
```{r}
combined_shrink_nocac_vec_lens <- get_vec_lens("combined_shrink_nocac",vars=c("dpiw_mu","fpiw_mu"),model_out_dir=model_out_dir)

cat("R\n")
get_ci_pd(draws = combined_shrink_nocac_vec_lens$theor_mn[,,"sv"])
cat("\nPE\n")
get_ci_pd(draws = combined_shrink_nocac_vec_lens$theor_mn[,,"pe"])
cat("\nCC\n")
get_ci_pd(draws = combined_shrink_nocac_vec_lens$theor_mn[,,"cc"])
```

# Vector-length recovery simulations (Study 2)
Simulate datasets under reward theory, PE theory, CC theory, and a combination of these theories (i.e., "blend" condition).
Then recover vector lengths with OLS (subject fixed effects).
```{r}
rec_dir <- paste0(path_to_project_directory,"output/results/vector_recovery/")
rec_tag <- "predvec250"

s2_vrec <- run_s2_vector_recovery(
  trials = trials,
  model_out_dir = model_out_dir,
  n_sims = 250,
  seed = 19
...
write.csv(s2_vrec$estimates,paste0(rec_dir,"s2_vector_recovery_",rec_tag,"_estimates.csv"),row.names=FALSE)

ggsave(paste0(fig_dir,"s2_vector_recovery_",rec_tag,"_panel.pdf"),s2_vrec$panel_plot,width=5.5,height=3.8,device=cairo_pdf,bg="white")
ggsave(paste0(fig_dir,"s2_vector_recovery_",rec_tag,"_reward.pdf"),s2_vrec$condition_plots[["reward"]],width=2.4,height=1.8,device=cairo_pdf,bg="white")
ggsave(paste0(fig_dir,"s2_vector_recovery_",rec_tag,"_pe.pdf"),s2_vrec$condition_plots[["pe"]],width=2.4,height=1.8,device=cairo_pdf,bg="white")
ggsave(paste0(fig_dir,"s2_vector_recovery_",rec_tag,"_cc.pdf"),s2_vrec$condition_plots[["cc"]],width=2.4,height=1.8,device=cairo_pdf,bg="white")
ggsave(paste0(fig_dir,"s2_vector_recovery_",rec_tag,"_blend.pdf"),s2_vrec$condition_plots[["blend"]],width=2.4,height=1.8,device=cairo_pdf,bg="white")
```
...
Get summary stats for the recovery
```{r}
zero_recovery_summary <- summarize_zero_recovery(
  length_df = s2_vrec$estimates,
  include_resid = FALSE
)

# Percentage of recovered lengths that are zero for vectors simulated with target length 0.
zero_recovery_summary$by_prediction
zero_recovery_summary$by_condition
zero_recovery_summary$overall

# Directional recovery summaries (underestimated vs overestimated).
over_under_summary <- summarize_over_under_recovery(
  length_df = s2_vrec$estimates,
  ref_df = vector_recovery_reference_lengths()
)
over_under_summary$positive_targets_overall
over_under_summary$positive_targets_by_prediction
over_under_summary$most_overestimated_prediction
```

Rebuild density plots from saved simulation outputs, without rerunning simulations.
```{r}
# replot_dot <- build_vector_recovery_plots_from_file(
#   estimates_file = paste0(rec_dir, "s2_vector_recovery_",rec_tag,"_estimates.csv"),
#   plot_type = "dot"
# )
# 
# replot_dot$panel_plot
# 
# # Recreate exact-zero recovery summaries from saved simulation outputs.
# zero_recovery_summary <- summarize_zero_recovery(
#   length_df = read.csv(paste0(rec_dir, "s2_vector_recovery_",rec_tag,"_estimates.csv"), stringsAsFactors = FALSE),
#   include_resid = FALSE
# )
# 
# zero_recovery_summary$by_prediction
...
# zero_recovery_summary$overall
# 
# # Recreate directional recovery summaries from saved simulation outputs.
# over_under_summary <- summarize_over_under_recovery(
#   length_df = read.csv(paste0(rec_dir, "s2_vector_recovery_",rec_tag,"_estimates.csv"), stringsAsFactors = FALSE),
#   ref_df = vector_recovery_reference_lengths()
# )
...
# Simplified outcome-theory recovery: aggregated variables vs vectors
Simulate a case in which only outcome-time basis variables influence valence, as follows:
`r_ch ~ N(0.5, 0.2)`, `Q_ch ~ N(0.25, 0.1)`, and `r_unch ~ N(0.25, 0.1)`.
Then compare recovery under the aggregated-variable approach and the vector-based approach.
```{r}
outcome_rec_tag <- "250"

# Run the simulation
s2_outcome_rec <- run_s2_outcome_theory_recovery(
  trials = trials,
  model_out_dir = model_out_dir,
  n_sims = 250,
  seed = 19
...
# Save the recovered theory-level estimates for later summaries or replotting.
write.csv(
  s2_outcome_rec$estimates,
  paste0(rec_dir, "s2_outcome_theory_recovery_", outcome_rec_tag, "_estimates.csv"),
  row.names = FALSE
)

# Export separate figures for the aggregated-variable and vector-based recoveries.
ggsave(
  paste0(fig_dir, "s2_outcome_theory_recovery_", outcome_rec_tag, "_aggregated.pdf"),
  s2_outcome_rec$aggregated_plot,
  width = 2.2,
  height = 1.8,
  device = cairo_pdf,
  bg = "white"
...
ggsave(
  paste0(fig_dir, "s2_outcome_theory_recovery_", outcome_rec_tag, "_vector.pdf"),
  s2_outcome_rec$vector_plot,
  width = 2.2,
  height = 1.8,
  device = cairo_pdf,
  bg = "white"
...
```{r}
# Aggregated-variable recovery
s2_outcome_rec$aggregated_plot
```

```{r}
# Vector-based recovery
s2_outcome_rec$vector_plot
```
