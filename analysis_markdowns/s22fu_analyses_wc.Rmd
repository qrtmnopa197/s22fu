...
quantile(mpan_nocac_scds[,,"scd_beta_mu"],c(.025,.5,.975))
mean(mpan_nocac_scds[,,"scd_beta_mu"] > 0)
quantile(mpan_nocac_scds[,,"scd_aff_sens_mu"],c(.025,.5,.975))
mean(mpan_nocac_scds[,,"scd_aff_sens_mu"] > 0)
quantile(mpan_nocac_scds[,,"scd_beta_mu"] - mpan_nocac_scds[,,"scd_aff_sens_mu"],c(.025,.5,.975))
mean(mpan_nocac_scds[,,"scd_beta_mu"] > mpan_nocac_scds[,,"scd_aff_sens_mu"])
```

Estimate no-autocorrelation vector lengths for reward, prediction error, counterfactual comparison, and residual.
```{r}
combined_shrink_nocac_vec_lens <- get_vec_lens("combined_shrink_nocac",vars=c("dpiw_mu","fpiw_mu"),model_out_dir=model_out_dir)

cat("SV\n")
quantile(combined_shrink_nocac_vec_lens$theor_mn[,,"sv"],c(.025,.5,.975))
cat("\nPE\n")
quantile(combined_shrink_nocac_vec_lens$theor_mn[,,"pe"],c(.025,.5,.975))
cat("\nCC\n")
quantile(combined_shrink_nocac_vec_lens$theor_mn[,,"cc"],c(.025,.5,.975))
cat("\nResidual\n")
quantile(combined_shrink_nocac_vec_lens$theor_mn[,,"resid"],c(.025,.5,.975))
```
