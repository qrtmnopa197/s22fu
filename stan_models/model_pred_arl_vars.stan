...
transformed data{
  ...
  int n_hp = n_dpi + n_fpi + 10; //number of hirearchical parameters
}
...
transformed parameters {
  ...
  //mean choice probability variances, with and without each choice predictor
  real<lower=0> cp_var_mu;
  real<lower=0> cp_var_noA_mu; 
  real<lower=0> cp_var_noQ_mu; 
  real<lower=0> cp_var_noC_mu; 
  {//anonymous_scope_start
    ...
    real feed_rate_model; //model-predicted feedback valence rating on this trial
    ...
    //choice probabilities and variances in choice probabilities
    vector[n_t] cp;
    vector[n_t] cp_noA;
    vector[n_t] cp_noQ;
    vector[n_t] cp_noC;
    ...
    vector[n_s] cp_var;
    vector[n_s] cp_var_noA;
    vector[n_s] cp_var_noQ;
    vector[n_s] cp_var_noC;
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        choice_lik[(s-1)*n_t+t] = categorical_logit_lpmf(choice[s,t] | beta[s]*Q[s,t,{fA[s,t],fB[s,t]}] + aff_sens[s]*A[s,t,{fA[s,t],fB[s,t]}] + phi[s]*C[s,t,{fA[s,t],fB[s,t]}]); //get the likelihood of the choice made on this trial.
       
        cp[t] = exp(choice_lik[(s-1)*n_t+t]);
        cp_noA[t] = exp(categorical_logit_lpmf(choice[s,t] | beta[s]*Q[s,t,{fA[s,t],fB[s,t]}] + phi[s]*C[s,t,{fA[s,t],fB[s,t]}]));
        cp_noQ[t] = exp(categorical_logit_lpmf(choice[s,t] | aff_sens[s]*A[s,t,{fA[s,t],fB[s,t]}] + phi[s]*C[s,t,{fA[s,t],fB[s,t]}]));
        cp_noC[t] = exp(categorical_logit_lpmf(choice[s,t] | beta[s]*Q[s,t,{fA[s,t],fB[s,t]}] + aff_sens[s]*A[s,t,{fA[s,t],fB[s,t]}]));
        ...
        feed_rate_model = base[s] + blk[s]*block_c[s,t] + tr[s]*tinb_c[s,t] + pr[s]*prev_rate[s,t] + dot_product(fpiw[s],fpi);
        ...
        if(t < n_t){
          ...
          if(choice[s,t] == 1){
            ...
            A[s,t+1,fA[s,t]] = A[s,t,fA[s,t]] + alpha[s]*(feed_rate_model - A[s,t,fA[s,t]]);
          } else if(choice[s,t] == 2){
            // vice-versa if fB was chosen
            choice_a = 0;
            choice_b = 1;
            A[s,t+1,fB[s,t]] = A[s,t,fB[s,t]] + alpha[s]*(feed_rate_model - A[s,t,fB[s,t]]);
          }
        ...
        if(dp_num[s,t] != 0){
          ...
          dpi[2] = Q[s,t,chosen_frac[s,t]]; //assign Q_chosen
          dpi[3] = Q[s,t,unchosen_frac[s,t]]; //assign Q_unchosen
          ...
          feed_pred[fp_num[s,t]] = feed_rate_model; //predict feedback rating
        }
      }
      cp_var[s] = variance(cp);
      cp_var_noA[s] = variance(cp_noA);
      cp_var_noQ[s] = variance(cp_noQ);
      cp_var_noC[s] = variance(cp_noC);
    }
    cp_var_mu = mean(cp_var);
    cp_var_noA_mu = mean(cp_var_noA);
    cp_var_noQ_mu = mean(cp_var_noQ);
    cp_var_noC_mu = mean(cp_var_noC);
  }//anonymous_scope_end
}
...
model {
  ...
  dpiw_mu ~ normal(0,shrink); //applying the shrinkage prior
  ...
  m_zs = append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(dpiw_z,fpiw_z),base_z),blk_z),alpha_z),beta_z),tau_z),phi_z),tr_z),pr_z),forget_z),aff_sens_z);
