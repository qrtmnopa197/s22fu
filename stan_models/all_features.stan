// Q-learning model with choice autocorrelation, forgetting (independent decay rate), and side bias. 
...
// NB: The valence predictors of interest (fpi and dpi) are the variables that contribute to the effects of interest, 
// rather than the  effects of interest themselves (e.g., unchosen outcome is used in lieu of (unchosen - chosen outcome)). 
// This model is formally equivalent to a combined model that predicts valence directly using the effects of interests, 
// and posteriors for the effects of interest can be computed from its fit. 
// This parameterization is preferable because it lessens collinearity between predictors and thus allows for more efficient model-fitting.
  ...
  {//anonymous_scope_start
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        choice_lik[(s-1)*n_t+t] = categorical_logit_lpmf(choice[s,t] | beta[s]*Q[s,t,{fA[s,t],fB[s,t]}] + phi[s]*C[s,t,{fA[s,t],fB[s,t]}] + [lsbias[s],0]'); //get the likelihood of the choice made on this trial.
...
model {
  ...
  m_zs = append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(dpiw_z,fpiw_z),base_z),blk_z),alpha_z),beta_z),tau_z),phi_z),tr_z),pr_z),lsbias_z),forget_z);
