...
// Additionally, this includes a linear model of valence ratings predicted by nuisance variables alone.
...
transformed data{
  int n_hp = 9; //number of hirearchical parameters
}
  ...
  {//anonymous_scope_start
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        if(dp_num[s,t] != 0){
          dec_pred[dp_num[s,t]] = base[s] + blk[s]*block_c[s,t] + tr[s]*tinb_c[s,t] + pr[s]*prev_rate[s,t]; //predict decision rating
        } else if(fp_num[s,t] != 0){
          feed_pred[fp_num[s,t]] = base[s] + blk[s]*block_c[s,t] + tr[s]*tinb_c[s,t] + pr[s]*prev_rate[s,t]; //predict feedback rating
        }
      }
    }
  }//anonymous_scope_end
...
model {
  ...
  m_zs = append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(blk_z,base_z),alpha_z),beta_z),tau_z),phi_z),tr_z),pr_z),forget_z);
