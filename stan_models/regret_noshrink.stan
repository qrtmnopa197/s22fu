...
transformed data{
  ...
  int n_fpi = 1; //Number of feedback predictors of interest
  ...
  {//anonymous_scope_start
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        if(dp_num[s,t] != 0){
          dpi[1] = Q[s,t,unchosen_frac[s,t]] - Q[s,t,chosen_frac[s,t]]; //assign Q_regret
          ...
          fpi[1] = unchosen_out[s,t] - chosen_out[s,t]; //regret
