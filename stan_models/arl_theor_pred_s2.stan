...
parameters {
  ...
  vector<lower=0>[4] len_sigma;
  matrix<lower=0>[n_s,4] len_z;
...
transformed parameters {
  ...
  {//anonymous_scope_start
    ...
    real feed_curr_pred; //model-predicted feedback valence rating on this trial
    real feed_nuis;
    ...
    fpiw[,1] = (len_sigma[1]*len_z[,1])*-0.5;
    fpiw[,2] = (len_sigma[2]*len_z[,2])*-0.5;
    fpiw[,3] = (len_sigma[3]*len_z[,3]) + (len_sigma[1]*len_z[,1])*0.5 + (len_sigma[2]*len_z[,2])*0.5 + (len_sigma[4]*len_z[,4])*0.5;
    fpiw[,4] = (len_sigma[4]*len_z[,4])*-0.5;
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        feed_curr_pred = base[s] + dot_product(fpiw[s],fpi);
        feed_nuis = blk[s]*block_c[s,t] + tr[s]*tinb_c[s,t] + pr[s]*prev_rate[s,t];
        ...
        if(dp_num[s,t] != 0){
          ...
          feed_pred[fp_num[s,t]] = feed_curr_pred + feed_nuis; //predict feedback rating
        }
        ...
        if(t < n_t){
          ...
          if(choice[s,t] == 1){
            ...
            A[s,t+1,fA[s,t]] = A[s,t,fA[s,t]] + alpha[s]*(feed_curr_pred - A[s,t,fA[s,t]]);
          } else if(choice[s,t] == 2){
            // vice-versa if fB was chosen
            choice_a = 0;
            choice_b = 1;
            A[s,t+1,fB[s,t]] = A[s,t,fB[s,t]] + alpha[s]*(feed_curr_pred - A[s,t,fB[s,t]]);
          }
...
model {
  ...
  dpiw_mu ~ normal(0,1); //applying the shrinkage prior
  dpiw_sigma ~ normal(0,2);
  
  len_sigma ~ normal(0,2);
  ...
  m_zs = append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(dpiw_z,len_z),base_z),blk_z),alpha_z),beta_z),tau_z),phi_z),tr_z),pr_z),forget_z),aff_sens_z);
