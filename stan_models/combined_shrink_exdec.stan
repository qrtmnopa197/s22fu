...
transformed data{
  int n_ep = 7; //Number of temporarlly extended valence predictors
  
  int fps = 4; //col of ex_preds on which the feedback predictors start
  int fpe = n_ep; //"" end
  
  int n_hp = n_ep + 9; //number of hirearchical parameters
...
transformed parameters {
  ...
  {//anonymous_scope_start
    ...
    array[n_s] matrix[n_t,n_ep] ex_preds = rep_array(rep_matrix(0,n_t,n_ep),n_s); //matrices of extended predictors
                                                                                     //one matrix per subject, with trials on the rows and predictor values in the columns
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        ex_preds[s,t,4] = chosen_out[s,t]*exp(choice_lik[(s-1)*n_t+t]) + unchosen_out[s,t]*(1-exp(choice_lik[(s-1)*n_t+t]));
        ex_preds[s,t,5] = Q[s,t,chosen_frac[s,t]]; //assign Q_chosen,feed.
        ex_preds[s,t,6] = chosen_out[s,t];
        ex_preds[s,t,7] = unchosen_out[s,t];
      }
    }
      ...
      for(t in 1:n_t){
        ...
        if(dp_num[s,t] != 0){
          if(t <= n_tb){
            ...
            dec_pred[dp_num[s,t]] = base[s] + blk[s]*block_c[s,t] + tr[s]*tinb_c[s,t] + dot_product(sum_preds,ew[s]);
          } else{
         ...
         //if there is a feedback probe on this timestep                                                                                                        //for this timestep
          ...
          if(t <= n_tb){
            ...
            feed_pred[fp_num[s,t]] = base[s] + blk[s]*block_c[s,t] + tr[s]*tinb_c[s,t] + dot_product(sum_preds,ew[s]);
          } else{
...
model {
  ...
  ew_mu ~ normal(0,shrink); //applying the shrinkage prior
  ew_sigma ~ normal(0,2);
  ...
  m_zs = append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(ew_z,gamma_z),base_z),blk_z),alpha_z),beta_z),tau_z),phi_z),tr_z),forget_z);
