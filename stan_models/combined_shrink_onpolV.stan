...
// Replaces V_block with a weighted average of Q-values
...
data {
  ...
  array[n_s,n_t,2,2] int pairs; // A 4d array containing the indices of the fractals in the current block.
                                // It contains the 2 fractals in each of the 2 pairs for each trial of each subject.
...
transformed parameters {
  ...
  {//anonymous_scope_start
    ...
    vector[2] pair_ev; //the EVs of the pairs in the current block
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        if(dp_num[s,t] != 0){
          for(p in 1:2){
            pair_ev[p] = dot_product(Q[s,t,pairs[s,t,p]],
                                     softmax(beta[s]*Q[s,t,pairs[s,t,p]] + phi[s]*C[s,t,pairs[s,t,p]])); 
          }
          dpi[1] = mean(pair_ev);
