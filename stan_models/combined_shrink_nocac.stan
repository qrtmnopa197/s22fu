// Predict choice with Q-learning model that includes forgetting
  ...
  {//anonymous_scope_start
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        choice_lik[(s-1)*n_t+t] = categorical_logit_lpmf(choice[s,t] | beta[s]*Q[s,t,{fA[s,t],fB[s,t]}]); //get the likelihood of the choice made on this trial.
        ...
        //unlesss this is the subject's last trial, set the Q values for the next trial
        if(t < n_t){
...
model {
  ...
  m_zs = append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(dpiw_z,fpiw_z),base_z),blk_z),alpha_z),beta_z),tr_z),pr_z),forget_z);
