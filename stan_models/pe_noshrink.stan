...
transformed data{
  int n_dpi = 1; //Number of decision preditors of interest
  int n_fpi = 2; //Number of feedback predictors of interest
  ...
  {//anonymous_scope_start
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        if(dp_num[s,t] != 0){
          dpi[1] = Q[s,t,chosen_frac[s,t]] - V[s,t]; //assign V_block
          ...
          fpi[1] = chosen_out[s,t] - (chosen_out[s,t]*exp(choice_lik[(s-1)*n_t+t]) + unchosen_out[s,t]*(1-exp(choice_lik[(s-1)*n_t+t]))); //V_trial
          fpi[2] = chosen_out[s,t] - Q[s,t,chosen_frac[s,t]]; //Q_chosen
