...
transformed parameters {
  ...
  {//anonymous_scope_start
    ...
    vector[n_s] vlr = inv_logit(vlr_mu + vlr_sigma*vlr_z);
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        if(t < n_t){
          ...
          V[s,t+1] = V[s,t] + vlr[s]*(chosen_out[s,t] - V[s,t]); //update V_block with whatever the outcome was
        }
...
model {
  ...
  m_zs = append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(dpiw_z,fpiw_z),base_z),blk_z),alpha_z),beta_z),tau_z),phi_z),tr_z),pr_z),forget_z),vlr_z);
