...
transformed data{
  ...
  int fps = 3; //col of ex_preds on which the feedback predictors start
  int fpe = n_ep; //"" end
  ...
  {//anonymous_scope_start
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        //Fill out ex_preds
        ex_preds[s,t,1] = Q[s,t,chosen_frac[s,t]] - V[s,t]; //assign V_block
        ex_preds[s,t,2] = Q[s,t,chosen_frac[s,t]] - Q[s,t,unchosen_frac[s,t]]; //assign Q_chosen,dec.
        //calculate V_trial as the average of the chosen and unchosen outcome, weighted by the probability of the choice made
        ex_preds[s,t,3] = chosen_out[s,t]*exp(choice_lik[(s-1)*n_t+t]) + unchosen_out[s,t]*(1-exp(choice_lik[(s-1)*n_t+t]));
