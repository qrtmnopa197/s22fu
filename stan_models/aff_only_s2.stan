...
data {
  ...
  array[n_s] vector[n_t] out_a; // 1d array of vectors - one for each subject - that each contains the outcomes 
                          // of fractal A for each of the subject's trial
...
model {
  ...
  m_zs = append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(append_col(dpiw_z,fpiw_z),base_z),blk_z),alpha_z),tau_z),phi_z),tr_z),pr_z),forget_z),aff_sens_z);
...
generated quantities{
  ...
  vector[n_fp] sim_feed_rat;
  {
    ...
    // Chosen and unchosen outcomes and fractals on the current trial
    real ch_out;
    real unch_out;
    int ch_frac;
    int unch_frac;
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        sim_prat[(s-1)*n_t+t] = curr_prat; //set previous rating
        ...
        //get feedback rating prediction
        fpi[1] = ch_out*exp(ch_lik) + unch_out*(1-exp(ch_lik)); //V_trial
        fpi[2] = Q[s,t,ch_frac]; //Q_chosen
        fpi[3] = ch_out; //reward_chosen
        fpi[4] = unch_out; //reward_unchosen
        feed_curr_pred = base[s] + dot_product(fpiw[s],fpi);
        feed_nuis = blk[s]*block_c[s,t] + tr[s]*tinb_c[s,t] + pr[s]*curr_prat;
        ...
        if(t < n_t){
          ...
          V[s,t+1] = V[s,t] + alpha[s]*(ch_out - V[s,t]); //update V_block with whatever the outcome was
        }
      }
    }
  }
