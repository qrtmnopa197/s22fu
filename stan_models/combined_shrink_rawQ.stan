// Q-learning model fit to choices.
...
transformed data{
  ...
  int n_hp = n_dpi + n_fpi + 6; //number of hirearchical parameters
}
  ...
  {//anonymous_scope_start
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        if(t < n_t){
          Q[s,t+1] = Q[s,t]; // Decay Q values of fractals not presented toward 0
...
model {
  ...
  m_zs = append_col(append_col(append_col(append_col(append_col(append_col(append_col(dpiw_z,fpiw_z),base_z),blk_z),alpha_z),beta_z),tr_z),pr_z);
